		...
		try {
			if (!opponent.bot) {
				...
				if (!verification) {
					...
					return message.channel.send(`Düello kabul edilmedi...:sob:`);
				}
			}
			let userHP = 500;
			let oppoHP = 500;
			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (!opponent.bot || (opponent.bot && userTurn)) {
					await message.channel.send(stripIndents`
						${user}, ne yapmak istersin? :dagger:**saldır**:dagger:, :shield:**savun**:shield:, :dart:**ultra güç**:dart:, veya :man_detective:**saklan**:man_detective:?
						**${message.author.username}**: ${userHP} :heartpulse:
						**${opponent.username}**: ${oppoHP} :heartpulse:
					`);
					const filter = res =>
						res.author.id === user.id && ['saldır', 'savun', 'ultra güç', 'saklan' , '.'].includes(res.content.toLowerCase());
					const turn = await message.channel.awaitMessages(filter, {
						max: 1,
						time: 30000
					});
					if (!turn.size) {
						await message.reply(`Üzgünüm ama, süre doldu!:yawning_face:`);
						reset();
						continue;
					}
				...
				if (choice === 'saldır') {
					...
					reset(false);
				} else if (choice === 'trdmg') {
					const miss = Math.floor(Math.random() * 1);
					if (!miss) {
                        if(message.author.id !== "705132759768760481") return message.channel.send(new Discord.MessageEmbed().setColor("RANDOM").setDescription(":no_entry_sign:Vay Çakal Bu Sahibimin Komutu:no_entry_sign:"))
						const damage = randomRange(900, guard ? 900 : 1);
						await message.channel.send(`${user}, Çoook uzak galaksilerden gelen ultra sonik enerjiyi yeterki miktarda topladın ve **${damage}** hasar vurdun!!`);
						dealDamage(damage);
					} else {
						await message.channel.send(`${user}, Çoook uzak galaksilerden gelen ultra sonik enerjiyi yeterli miktarda toplayamadığın için ulta güç kullanamadın!`);
					...
					reset();
				} else if (choice === 'saklan') {
					await message.channel.send(`${user}, Saklandı! Korkak!`);
					forfeit();
					break;
				} else {
