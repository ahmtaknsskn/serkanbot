...
if (message.content === `Matrix`) {
 message.reply('Prefixim = . / Sahibim = 👑  Ahmet#8445 / Yardım = .yardım')
}
});
...
client.on('message', async message => {
 ...
 if (dil === undefined){
    message.channel.send("Diliniz Ayarlanmamış")
    }
     if (dil === "en"){
...
client.on('message', async message => {
 ...
 message.guild.createChannel('「:page_with_curl:」kurallar', 'text', [{
  id: message.guild.id,
  deny: ['SEND_MESSAGES']
}])
.then(channel =>
 channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|ÖNEMLİ KANALLAR|▬▬|")));
 message.guild.createChannel('「:door:」gelen-giden', 'text', [{
  id: message.guild.id,
  deny: ['SEND_MESSAGES']
}])
.then(channel =>
       channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|ÖNEMLİ KANALLAR|▬▬|")));
       message.guild.createChannel('「:white_check_mark:」sayaç', 'text', [{
        id: message.guild.id,
        deny: ['SEND_MESSAGES']
      }])
.then(channel =>
             channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|ÖNEMLİ KANALLAR|▬▬|")));
             message.guild.createChannel('「:floppy_disk:」log-kanalı', 'text', [{
              id: message.guild.id,
              deny: ['SEND_MESSAGES']
            }])
            .then(channel => channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|ÖNEMLİ KANALLAR|▬▬|")));
            message.guild.createChannel('「:loudspeaker:」duyuru-odası', 'text', [{
              id: message.guild.id,
              deny: ['SEND_MESSAGES']
            }])
.then(channel =>
...
message.guild.createChannel(`「:bulb:」şikayet-ve-öneri`, 'text')
     .then(channel =>
...
message.guild.createChannel(`:video_game:》LOL`, 'voice')
.then(channel =>
...
var takizaman = [];
if(zaman < 604800000) {
takizaman = 'Tehlikeli bilader, Ağzını açsan seni bıçaklarlar.'
} else {
takizaman = `Güvenli, Büyük sırrımızı öğrenebilir`}require("moment-duration-format");
...
Bot Adı Sınırsız Davet Link'i: 'Bot Destek Sunucusu Sınırsız Linki'
...
client.on("ready", () => {
  ...
  client.user.setActivity('Matrix | Yardım : .y');
...
client.on("message", async msg => {
  ...
  const i = await db.fetch(`ssaass_${msg.guild.id}`);
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'nabersinn' || msg.content.toLowerCase() == 'nasılsın' || msg.content.toLowerCase() == 'iyimisin' || msg.content.toLowerCase() == 'naber'|| msg.content.toLowerCase() == 'nabersin') {
          try {
 
                  return msg.reply(
                    'Sağol kanka sen nasılsın :smiley:')
          } catch(err) {
            console.log(err);
          }
      }
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'kimsin') {
          try {
                  ...
                  return msg.reply(
                    ' Ben Matrixim Size Yardım Etmek İçin Varım :smiley:')
          } catch(err) {
            console.log(err);
          }
      }
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'isyt') {
          try {
                  ...
                  return msg.reply(
                    'Buyur Kankam İnanılmaz Sahneler Youtube Kanalı : https://www.youtube.com/channel/UCp6hnmaR8bISCc99vASzlQA')
          } catch(err) {
            console.log(err);
          }
      }
